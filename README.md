...
La medición está automatizada en [`Pruebas/benchmark.sh`](Pruebas/benchmark.sh): levanta la base desde cero, extrae las consultas de `04` y los `CREATE INDEX` de `05` de los propios scripts, mide cada una con `EXPLAIN (ANALYZE, BUFFERS)` y deja los planes completos en `Pruebas/Evidencias_Rendimiento/`. Se tomó el mejor tiempo de quince corridas por consulta. Las quince repeticiones vienen del dataset chico original, donde todo terminaba en menos de un milisegundo y con tres el ruido de medición era del mismo orden que la diferencia a medir; a esta escala las consultas tardan entre 89 y 318 ms y el ruido pesa mucho menos. Dos corridas seguidas de quince repeticiones dieron los mismos tiempos dentro del 2,2 %; entre corridas de días distintos, con la misma base y los mismos planes, la diferencia llegó al 11 %.

Entorno: PostgreSQL 16.15 en Docker con la configuración por defecto de la imagen (`shared_buffers` de 128 MB, `work_mem` de 4 MB, hasta dos workers paralelos por consulta), en un AMD Ryzen 7 8845HS con 14 GB de RAM. Medido el 23 de septiembre de 2026 con `REPETICIONES=15 Pruebas/benchmark.sh`; la corrida completa, carga incluida, tardó unos 71 segundos.
...
|---|---|---|---|---|---|---|
| 1 | Recaudación por marca, salidas del último trimestre | 4 | 104.377 | 88.690 | +15 % | `idx_movimientos_tipo_fecha` |
| 2 | Variantes sin salidas en el año | 450 | 130.106 | 108.530 | +17 % | `idx_detalle_movimientos_id_mov` |
| 3 | Tickets y recaudación por empleado en Sucursal NOA | 5 | 141.726 | 89.502 | +37 % | `idx_detalle_movimientos_id_mov`, `idx_movimientos_id_sucursal_origen` |
| 4 | Variantes con stock bajo | 147323 | 153.395 | 148.709 | +3 % | ninguno, sigue con Seq Scan |
| 5 | Ventas por marca y sucursal | 32 | 311.011 | 317.553 | -2 % | ninguno, sigue con Seq Scan |
